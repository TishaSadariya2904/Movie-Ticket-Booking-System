...
class TheaterPageController extends Controller
{
    ...
    {
        $theater = $this->theatre->getTheatre($id);
        $city=$this->city->getCityById($id);
        return View::make('admin.Theater.edit', compact('theater','city'));
    }
    ...
    public function store(Request $request, $id = null)
    {
        ...
        if (!is_null($id)) {
            $this->theatre->createOrUpdateTheater($id, $collection);
        } else {
            $this->theatre->createOrUpdateTheater($id = null, $collection);
        }
